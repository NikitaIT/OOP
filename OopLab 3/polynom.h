...
public:
  ...
  Matrix calculateValue(Matrix x){
  ...
  complex<double> calculateValue(complex<double> x){

      return PolinomGorner(*this->Koefs,x);
  }

  static value_type PolinomGorner(vector<Polynom::value_type,Polynom::allocator_type> &a, Polynom::value_type x)
  {
    int n = a.size() - 1;
    value_type y = a[n];
