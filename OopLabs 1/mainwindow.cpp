      ...
      if (rx.indexIn(n) >= 0) {
          if(rx.cap(2)!="i"){
              if(ii.indexIn(rx.cap(3))>= 0){
                  com->real(rx.cap(1).toDouble(&ok1));
                  com->imag(rx.cap(3).toDouble(&ok2));
              } else{
                    com->real(rx.cap(1).toDouble(&ok1)+rx.cap(3).toDouble(&ok2));
                }
          } else{
              QRegExp rdx("([+|-]?[i]?[0-9.,]+[i]?)([a-zA-Z0-9]*)([+-]?[0-9.,]+)");

              if (rdx.indexIn(n) >= 0) {
                  if(ii.indexIn(rx.cap(1))>= 0){
                      com->real( rdx.cap(3).toDouble(&ok1));
                      com->imag( rdx.cap(1).toDouble(&ok2));
                    } else{
                      ok1 = ok2=false;
                    }
                }
