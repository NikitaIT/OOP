...
template<typename T>
bool QSMatrix<T>::operator==(QSMatrix<T> &rhs) const
{
  //if(rhs == nullptr || this == nullptr){
  //  return false;
  //}
...
T QSMatrix<T>::det()
{
    auto B = *new QSMatrix<T>(this);
    //приведение матрицы к верхнетреугольному виду
    for(int step = 0; step < this->get_rows() - 1; step++)
        for(int row = step + 1; row < this->get_rows(); row++)
        {
            double coeff = -B(row,step) / B(step,step); //метод Гаусса
            for(int col = step; col < this->get_cols(); col++)
